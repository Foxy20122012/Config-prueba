...
const environment = {
  ...
  getEnvUser: () => {
    return new Promise((resolve, reject) => {
      const token = Cookie.get(`${publicRuntimeConfig.idApp}:${publicRuntimeConfig.NODE_ENV}`);
      const user = functions.getDecodeStorage(`${publicRuntimeConfig.idApp}/${publicRuntimeConfig.NODE_ENV}`);
      if (!token || !user) {
  ...
  logout: () => {
    return new Promise((resolve, reject) => {
      Cookie.remove(`${publicRuntimeConfig.idApp}:${publicRuntimeConfig.NODE_ENV}`);
      functions.setEncodeStorage(`${publicRuntimeConfig.idApp}/${publicRuntimeConfig.NODE_ENV}`, '');
      resolve('/');
    });
  },
  getTime: () => {
